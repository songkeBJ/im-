...
public class WeiXinPayController2 extends ResponsePage  {

	private Logger log = LoggerFactory.getLogger(WeiXinPayController2.class);
	...
	public String doWeinXinRequest(@RequestParam(value="acount",required=false) String acount,
		...
		String notify_url = address.getHostAddress()+"/SellerNet/weixin/payCallback.do";
		...
		if (!"".equals(token)) {
			...
			if(Constant.orderActive.RECHARGE.name().equals(type)){
				packageReqHandler.setParameter("body", "账号充值"); //商品描述   
			}
